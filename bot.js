...
CASPER.then(function(data) {
		...
		CASPER.eachThen(SITES, function(site) {
			this.site = site.data;
			this.site.link = this.site.protocol + this.site.domain + this.site.path;
			//this.echo(this.site.link,'INFO');
			
		  	this.thenOpen(this.site.link, function( headers ) {
		  		
				CASPER.waitForSelector(this.site.item_selector, function() {
					this.site.items = this.evaluate(function(site) {
						
						var itemsindex = [];
						var items = [];
						$(site.item_selector).each(function(){
							...
							///////////////////////////////////////////////////////////////////
							// ITEMS
							//if (item.img.src || item.link || item.body)) {
								// filter
								if (item.body) {
									item.link = uu.parseUrl(item.body);
								}
								// save
								items.push(item);
							//}
