...
// same bot.js as go.js
	...
	if (CASPER.cli.has("live")) { // override local dev environment
		CONFIG.live = CASPER.cli.get("live");
	} else {
		if (OS.name=='mac') {
...
*/
var EACH = {crawled:{}};
CASPER.start(CONFIG.api_host+'/site', function(headers) {
	...
	if (!site) {
		CASPER.console.error('no site data');
		return false;
	}
